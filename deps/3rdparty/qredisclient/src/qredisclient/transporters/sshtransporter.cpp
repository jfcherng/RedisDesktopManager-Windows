...
RedisClient::SshTransporter::SshTransporter(RedisClient::Connection* c)
    : RedisClient::AbstractTransporter(c), m_socket(nullptr), m_isHostKeyAlreadyAdded(false)
{
}
...
void RedisClient::SshTransporter::OnSshConnectionError(QSshClient::Error error)
{
    ...
    emit errorOccurred(
        QString("SSH Connection error(%1): %2").arg(getSshErrorString(error)).arg(m_sshClient->getLastError()));
}
